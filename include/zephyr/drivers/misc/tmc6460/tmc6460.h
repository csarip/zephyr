 ...
 */
static inline uint32_t tmc6460_field_update(uint32_t reg_val, struct tmc6460_field field,
					    uint32_t field_val)
{
	reg_val &= ~field.mask;
 ...
 * @retval -errno on failure.
 */
static inline int tmc6460_field_write(const struct device *dev, struct tmc6460_field field,
				      uint32_t field_val)
{
	uint32_t reg_val;
	int err;
