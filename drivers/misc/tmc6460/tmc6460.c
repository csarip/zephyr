...
static const uint8_t tmc6460_crc_table[256] = {
	,
};
...
static int tmc6460_spi_transceive(const struct device *dev, uint8_t *tx, uint8_t *rx, size_t len)
{
...
static int tmc6460_spi_write(const struct device *dev, uint16_t reg_addr, uint32_t reg_val)
{
...
static int tmc6460_spi_read(const struct device *dev, uint16_t reg_addr, uint32_t *reg_val)
{
	struct tmc6460_data *data = dev->data;
	uint8_t tx[TMC6460_SPI_DATAGRAM_SIZE] = {0};
	uint8_t rx[TMC6460_SPI_DATAGRAM_SIZE] = {0};
	...
	*reg_val = ((uint32_t)rx[2] << 24) | ((uint32_t)rx[3] << 16) | ((uint32_t)rx[4] << 8) |
		   ((uint32_t)rx[5]);
...
static int tmc6460_uart_tx(const struct device *uart, const uint8_t *buf, size_t len)
{
...
static int tmc6460_uart_rx(const struct device *uart, uint8_t *buf, size_t len)
{
	for (size_t i = 0; i < len; i++) {
		k_timepoint_t end = sys_timepoint_calc(K_MSEC(TMC6460_UART_RX_TIMEOUT_MS));
		int err;
...
static int tmc6460_uart_write(const struct device *dev, uint16_t reg_addr, uint32_t reg_val)
{
	const struct tmc6460_config *config = dev->config;
	struct tmc6460_data *data = dev->data;
	 ...
	 */
	buf[0] = TMC6460_UART_SYNC | TMC6460_UART_WRITE_BIT | ((reg_addr >> 4) & 0x30U);
...
static int tmc6460_uart_read(const struct device *dev, uint16_t reg_addr, uint32_t *reg_val)
{
	const struct tmc6460_config *config = dev->config;
	struct tmc6460_data *data = dev->data;
	...
	while (true) {
		...
		if (tmc6460_is_rtmi_header(rx_buf[0])) {
			...
			err = tmc6460_uart_rx(config->uart, &rx_buf[1], rtmi_size - 1);
			if (err < 0) {
				k_sem_give(&data->sem);
				return err;
			}
			...
			if (config->use_crc) {
				uint8_t crc = tmc6460_crc8(rx_buf, rtmi_size - 1);
		...
		err = tmc6460_uart_rx(config->uart, &rx_buf[1], read_size - 1);
		if (err < 0) {
			k_sem_give(&data->sem);
			return err;
		}
	...
	if (config->use_crc) {
		...
		if (crc != rx_buf[read_size - 1]) {
			LOG_ERR("UART read CRC mismatch for reg 0x%04x", reg_addr);
			return -EIO;
		}
	}
	 ...
	 * Byte 6 (optional): CRC8
	 */
	*reg_val = ((uint32_t)rx_buf[2] << 24) | ((uint32_t)rx_buf[3] << 16) |
		   ((uint32_t)rx_buf[4] << 8) | ((uint32_t)rx_buf[5]);
...
int tmc6460_rtmi_stream_write(const struct device *dev, uint8_t rtmi_index, uint32_t value)
{
	...
	if (err < 0) {
		LOG_ERR("RTMI stream write index %u failed: %d", rtmi_index, err);
	}
...
int tmc6460_poll_flag(const struct device *dev, uint16_t reg_addr, uint32_t flag_mask,
...
#define TMC6460_CONFIG_UART(inst)                                                                  \
	.uart = DEVICE_DT_GET(DT_INST_BUS(inst)), .use_crc = DT_INST_PROP_OR(inst, uart_crc, false),
#endif
...
#define TMC6460_DEFINE(inst)                                                                       \
	static struct tmc6460_data tmc6460_data_##inst;                                            \
                                                                                                   \
	static const struct tmc6460_config tmc6460_config_##inst = {TMC6460_BUS_CONFIG(inst)};     \
                                                                                                   \
	DEVICE_DT_INST_DEFINE(inst, tmc6460_init, NULL, &tmc6460_data_##inst,                      \
			      &tmc6460_config_##inst, POST_KERNEL, CONFIG_TMC6460_INIT_PRIORITY,   \
			      NULL);
