...
int max2221x_enable_part(const struct device *dev)
{
	const struct misc_max2221x_config *config = dev->config;
...
int max2221x_disable_part(const struct device *dev)
{
	const struct misc_max2221x_config *config = dev->config;
...
int max2221x_read_status(const struct device *dev, uint16_t *status)
{
	...
	if (*status & MAX2221X_STATUS_STT3_MASK) {
		LOG_INF("Channel 3: I_AC < I_AC_THLD");
	} else {
		LOG_INF("Channel 3: I_AC > I_AC_THLD");
	}

	if (*status & MAX2221X_STATUS_STT2_MASK) {
		LOG_INF("Channel 2: I_AC < I_AC_THLD");
	} else {
		LOG_INF("Channel 2: I_AC > I_AC_THLD");
	}

	if (*status & MAX2221X_STATUS_STT1_MASK) {
		LOG_INF("Channel 1: I_AC < I_AC_THLD");
	} else {
		LOG_INF("Channel 1: I_AC > I_AC_THLD");
	}

	if (*status & MAX2221X_STATUS_STT0_MASK) {
		LOG_INF("Channel 0: I_AC < I_AC_THLD");
	} else {
		LOG_INF("Channel 0: I_AC > I_AC_THLD");
	}

	if (*status & MAX2221X_STATUS_MIN_T_ON_MASK) {
		LOG_INF("MIN_T_ON not compliant");
	}

	if (*status & MAX2221X_STATUS_RES_MASK) {
		LOG_INF("Measured resistance not compliant");
	}

	if (*status & MAX2221X_STATUS_IND_MASK) {
		LOG_INF("Measured inductance not compliant");
	}

	if (*status & MAX2221X_STATUS_OVT_MASK) {
		LOG_INF("Over-temperature detected");
	}
	...
	if (*status & MAX2221X_STATUS_OLF_MASK) {
		LOG_INF("Open-loop detected");
	}

	if (*status & MAX2221X_STATUS_HHF_MASK) {
		LOG_INF("Hit current not reached");
	}

	if (*status & MAX2221X_STATUS_DPM_MASK) {
		LOG_INF("Plunger did not move");
	}
	...
	if (*status & MAX2221X_STATUS_UVM_MASK) {
		LOG_INF("Under-voltage detected");
	}
...
int max2221x_enable_ramps(const struct device *dev, uint8_t channel, uint8_t ramp_mask)
{
	const struct misc_max2221x_config *config = dev->config;
	...
	if (ramp_mask & MAX2221X_RAMP_DOWN_MASK) {
		return max2221x_reg_update(config->parent, MAX2221X_REG_CFG_CTRL0(channel),
					   MAX2221X_RDWE_MASK, 1);
	}

	if (ramp_mask & MAX2221X_RAMP_MID_MASK) {
		return max2221x_reg_update(config->parent, MAX2221X_REG_CFG_CTRL0(channel),
					   MAX2221X_RMDE_MASK, 1);
	}

	if (ramp_mask & MAX2221X_RAMP_UP_MASK) {
		return max2221x_reg_update(config->parent, MAX2221X_REG_CFG_CTRL0(channel),
					   MAX2221X_RUPE_MASK, 1);
	}

	return 0;
}

int max2221x_disable_ramps(const struct device *dev, uint8_t channel, uint8_t ramp_mask)
{
	const struct misc_max2221x_config *config = dev->config;
	...
	if (ramp_mask & MAX2221X_RAMP_DOWN_MASK) {
		return max2221x_reg_update(config->parent, MAX2221X_REG_CFG_CTRL0(channel),
					   MAX2221X_RDWE_MASK, 0);
	}

	if (ramp_mask & MAX2221X_RAMP_MID_MASK) {
		return max2221x_reg_update(config->parent, MAX2221X_REG_CFG_CTRL0(channel),
					   MAX2221X_RMDE_MASK, 0);
	}

	if (ramp_mask & MAX2221X_RAMP_UP_MASK) {
		return max2221x_reg_update(config->parent, MAX2221X_REG_CFG_CTRL0(channel),
					   MAX2221X_RUPE_MASK, 0);
	}
...
int max2221x_read_fault0(const struct device *dev)
{
	...
	if (reg & MAX2221X_FAULT_DPM3_MASK) {
		LOG_INF("Channel 3: Plunger did not move");
	}

	if (reg & MAX2221X_FAULT_DPM2_MASK) {
		LOG_INF("Channel 2: Plunger did not move");
	}

	if (reg & MAX2221X_FAULT_DPM1_MASK) {
		LOG_INF("Channel 1: Plunger did not move");
	}

	if (reg & MAX2221X_FAULT_DPM0_MASK) {
		LOG_INF("Channel 0: Plunger did not move");
	}

	if (reg & MAX2221X_FAULT_OLF3_MASK) {
		LOG_INF("Channel 3: Open-loop detected");
	}

	if (reg & MAX2221X_FAULT_OLF2_MASK) {
		LOG_INF("Channel 2: Open-loop detected");
	}

	if (reg & MAX2221X_FAULT_OLF1_MASK) {
		LOG_INF("Channel 1: Open-loop detected");
	}

	if (reg & MAX2221X_FAULT_OLF0_MASK) {
		LOG_INF("Channel 0: Open-loop detected");
	}

	if (reg & MAX2221X_FAULT_HHF3_MASK) {
		LOG_INF("Channel 3: Hit current not reached");
	}

	if (reg & MAX2221X_FAULT_HHF2_MASK) {
		LOG_INF("Channel 2: Hit current not reached");
	}

	if (reg & MAX2221X_FAULT_HHF1_MASK) {
		LOG_INF("Channel 1: Hit current not reached");
	}

	if (reg & MAX2221X_FAULT_HHF0_MASK) {
		LOG_INF("Channel 0: Hit current not reached");
	}

	if (reg & MAX2221X_FAULT_OCP3_MASK) {
		LOG_INF("Channel 3: Over-current detected");
	}

	if (reg & MAX2221X_FAULT_OCP2_MASK) {
		LOG_INF("Channel 2: Over-current detected");
	}

	if (reg & MAX2221X_FAULT_OCP1_MASK) {
		LOG_INF("Channel 1: Over-current detected");
	}

	if (reg & MAX2221X_FAULT_OCP0_MASK) {
		LOG_INF("Channel 0: Over-current detected");
	}
...
int max2221x_read_fault1(const struct device *dev)
{
	...
	if (reg & MAX2221X_FAULT_RES3_MASK) {
		LOG_INF("Channel 3: Measured resistance not compliant");
	}

	if (reg & MAX2221X_FAULT_RES2_MASK) {
		LOG_INF("Channel 2: Measured resistance not compliant");
	}

	if (reg & MAX2221X_FAULT_RES1_MASK) {
		LOG_INF("Channel 1: Measured resistance not compliant");
	}

	if (reg & MAX2221X_FAULT_IND3_MASK) {
		LOG_INF("Channel 3: Measured inductance not compliant");
	}
	...
	if (reg & MAX2221X_FAULT_IND2_MASK) {
		LOG_INF("Channel 2: Measured inductance not compliant");
	}

	if (reg & MAX2221X_FAULT_IND1_MASK) {
		LOG_INF("Channel 1: Measured inductance not compliant");
	}

	if (reg & MAX2221X_FAULT_IND0_MASK) {
		LOG_INF("Channel 0: Measured inductance not compliant");
	}
...
int max2221x_start_rapid_fire(const struct device *dev, uint8_t channel)
{
	...
	mask = MAX2221X_CNTL0_MASK << channel;
	...
	for (i = 0; i < data->repetitions[channel]; i++) {
		ret = max2221x_enable_channel(dev, channel);
		if (ret) {
			return ret;
		}
		...
		ret = max2221x_disable_channel(dev, channel);
		if (ret) {
			return ret;
		}
	...
	if (!data->stop_state[channel]) {
		return max2221x_enable_channel(dev, channel);
	}
...
int max2221x_stop_rapid_fire(const struct device *dev, uint8_t channel)
{
	...
	return max2221x_disable_channel(dev, channel);
}
...
static int misc_max2221x_init(const struct device *dev)
{
	...
	LOG_INF("MAX2221X Misc Initialized");
...
#define MISC_MAX2221X_DEFINE(inst)                                                                 \
	static const struct misc_max2221x_config misc_max2221x_config_##inst = {                   \
		.parent = DEVICE_DT_GET(DT_INST_PARENT(inst)),                                     \
                                                                                                   ...
                                                                                                   \
	DEVICE_DT_INST_DEFINE(inst, misc_max2221x_init, NULL, NULL, &misc_max2221x_config_##inst,  \
			      POST_KERNEL, CONFIG_MISC_MAX2221X_INIT_PRIORITY, NULL);
